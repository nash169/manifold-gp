...
v = (X_sampled[i[0, :], :] - X_sampled[i[1, :], :]
     ).pow(2).sum(dim=1).div(-lp_knn.eps_).exp()
...
L = (torch.sparse_coo_tensor(index_diag, torch.ones(neighbors.shape[0]), (
    neighbors.shape[0], neighbors.shape[0])).to(device) - torch.sparse.mm(D, L))/(1/4*lp_knn.eps_)

# # L = L.to_dense() + 2*lp_knn.nu_/lp_knn.k_**2*torch.eye(neighbors.shape[0])

model = MaternPrecision(X_sampled, k, lp_knn.nu_)
