...
kernel = gpytorch.kernels.ScaleKernel(RiemannMaternKernel(
    nu=nu, nodes=noisy_x, neighbors=neighbors, modes=modes))
...
with torch.no_grad(), gpytorch.settings.fast_pred_var():
    ...
    # test_x, faces, test_y = groundtruth_from_mesh(
    ...
    kernel_eval = kernel(noisy_x[point, :].unsqueeze(
        0), noisy_x).evaluate().squeeze().cpu().numpy()
    ...
    # Bring data to numpy
    test_x = sampled_x.cpu().numpy()
    test_y = sampled_y.cpu().numpy()

v_options = {'mode': 'sphere', 'scale_factor': 3e-3, 'color': (0, 0, 0)}

# Ground Truth
mlab.figure('Groud Truth', fgcolor=(0, 0, 0), bgcolor=(1, 1, 1))
mlab.triangular_mesh(test_x[:, 0], test_x[:, 1],
                     test_x[:, 2], faces, scalars=test_y)
mlab.colorbar(orientation='vertical')

# Mean
mlab.figure('Mean', fgcolor=(0, 0, 0), bgcolor=(1, 1, 1))
mlab.triangular_mesh(test_x[:, 0], test_x[:, 1],
                     test_x[:, 2], faces, scalars=mean)
mlab.colorbar(orientation='vertical')

# Standard Deviation
mlab.figure('Standard Deviation', fgcolor=(0, 0, 0), bgcolor=(1, 1, 1))
mlab.triangular_mesh(test_x[:, 0], test_x[:, 1],
                     test_x[:, 2], faces, scalars=std)
mlab.colorbar(orientation='vertical')

# One Posterior Sample
mlab.figure('One Posterior Sample', fgcolor=(0, 0, 0), bgcolor=(1, 1, 1))
mlab.triangular_mesh(test_x[:, 0], test_x[:, 1],
                     test_x[:, 2], faces, scalars=posterior_sample)
mlab.colorbar(orientation='vertical')
...
mlab.triangular_mesh(test_x[:, 0], test_x[:, 1],
                     test_x[:, 2], faces, scalars=kernel_eval)
mlab.colorbar(orientation='vertical')
mlab.points3d(test_x[point, 0], test_x[point, 1],
              test_x[point, 2], **v_options)
mlab.view(0.0, 180.0, 0.5139171204775793)

# Eigenfunction
eigfun = eigfunctions[mode, :] - np.min(eigfunctions[mode, :])
eigfun /= np.max(eigfun)
mlab.figure('Eigenfunction', ize=(1920, 1360),
            fgcolor=(0, 0, 0), bgcolor=(1, 1, 1))
mlab.triangular_mesh(test_x[:, 0], test_x[:, 1],
                     test_x[:, 2], faces, scalars=eigfun)
mlab.colorbar(orientation='vertical')
mlab.view(0.0, 180.0, 0.5139171204775793)
